...
function Combat:getDamage(attacker, defender, solveType, isCounter, attackerPos, defenderPos, attackerPath, isGroove, grooveWeaponIdOverride)
	...
	if solveType == "random" and Wargroove.isRNGEnabled() and not attacker.unitClass.isStructure and not defender.unitClass.isStructure then
	...
	if solveType == "simulationOptimistic" and not attacker.unitClass.isStructure and not defender.unitClass.isStructure then
		if isCounter then
			randomValue = 0
		else
			randomValue = 1
	...
	if solveType == "simulationPessimistic" and not attacker.unitClass.isStructure and not defender.unitClass.isStructure then
		if isCounter then
			randomValue = 1
		else
			randomValue = 0
